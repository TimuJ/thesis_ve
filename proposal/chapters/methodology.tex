...
\begin{figure}[h]
\centering
\includegraphics[width=0.95\textwidth]{figures/fig4_lr_vcc_architecture.png}
\caption{LR-VCC architecture: a super-resolved video as input; five sub-metrics computed in
parallel, each emitting a (score, reliability) pair in $[0,1]\times[0,1]$; a
reliability-weighted softmax-log-mean composition produces the final LR-VCC score in $[0,1]$.}
...
Section~\ref{sec:problem}; they are not tuned to the validation set.
...
evaluated on the real super-resolution test set comprising both super-resolution baselines.
The pass criterion is that the detail-preserving baseline wins on the per-method mean and on
every per-video comparison. The current production version meets this criterion: the
per-video comparison is consistent across all test videos including the regime-shift case,
with a mean difference between the two methods of approximately $+0.06$ in favour of the
...
values are re-aggregated through the composition layer in seconds. Additional sub-metrics
can be added to the composition without disturbing the cached values of the existing
sub-metrics.
