...
\begin{itemize}
    ...
    \item \textbf{VBench} \cite{huang_vbench_2024} and \textbf{VBench-2.0}
          ...
          including \texttt{subject\_consistency} and \texttt{background\_consistency}
          (DINOv2 cosine similarity between adjacent frames), \texttt{Human\_Identity}
          (slow-fast ArcFace clip adapter), and \texttt{Human\_Anatomy} (anomaly ViT). The
          preliminary work documents applicability gaps for super-resolution evaluation:
          DINOv2-based consistency metrics exhibit smoother-output bias and
          \texttt{Human\_Anatomy} has content-dependent calibration failure on close-up
          footage.
\end{itemize}
